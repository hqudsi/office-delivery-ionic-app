...
@Component({
  selector: 'app-drivers-collection',
  templateUrl: './drivers-collection.page.html',
  styleUrls: ['./drivers-collection.page.scss'],
})
export class DriversCollectionPage implements OnInit, ViewDidEnter {
  ...
  async loadData(refresh = false, refresher?) {
    ...
    this.apiService.getApi(refresh, 'my-drivers-collection', '/logistics/getDriversCollection', dataRequest).subscribe(async (res: any) => {
  ...
  loadMoreItems(event) {
    ...
    if (page > this.vouchersCollection.last_page) {
      ...
      this.apiService.getApi(true, 'my-drivers-collection', '/logistics/getDriversCollection', dataRequest).subscribe(async (res: any) => {
        if (res) {
          this.vouchersCollection = res;
          if (this.vouchers) {
